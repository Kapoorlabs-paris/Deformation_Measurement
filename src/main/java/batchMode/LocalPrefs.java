...
public class LocalPrefs {
	...
	static Properties ETrackPrefs = new Properties();
	static Properties props = new Properties(ETrackPrefs);
	...
	*/
	...
	public static void set(String key, String text) {
		...
		if (text==null)
			ETrackPrefs.remove(KEY_PREFIX+key);
		else
			ETrackPrefs.put(KEY_PREFIX+key, text);
	}
	...
	public static String get(String key, String defaultValue) {
		String value = ETrackPrefs.getProperty(KEY_PREFIX+key);
		if (value == null)
			return defaultValue;
		else
			return value;
	...
	public static double get(String key, double defaultValue) {
		String s = ETrackPrefs.getProperty(KEY_PREFIX+key);
		Double d = null;
		if (s!=null) {
			try {d = new Double(s);}
			catch (NumberFormatException e) {d = null;}
	...
	static void savePluginPrefs(Properties prefs) {
		Enumeration e = ETrackPrefs.keys();
		while (e.hasMoreElements()) {
			String key = (String) e.nextElement();
			if (key.indexOf(KEY_PREFIX) == 0)
				prefs.put(key, ETrackPrefs.getProperty(key));
		}
	}
