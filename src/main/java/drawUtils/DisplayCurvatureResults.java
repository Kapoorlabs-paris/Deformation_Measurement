...
public class DisplayCurvatureResults {
	...
	public static  RandomAccessibleInterval<FloatType>  Display(InteractiveSimpleEllipseFit parent, RandomAccessibleInterval<FloatType> originalimg, 
			ArrayList<Pair<String, Pair< Integer,ArrayList<double[]>>>> currentresultCurv) {
		
		RandomAccessibleInterval<FloatType> probImg = new ArrayImgFactory<FloatType>().create(originalimg, new FloatType());
		...
		//			parent.thirdDimensionSize, 1, parent.fourthDimensionSize);
			...
			while(cursor.hasNext()) {
				...
				if ( (Math.abs(cursor.getFloatPosition(0) - X[i])) < 2 && (Math.abs(cursor.getFloatPosition(1) - Y[i])) < 2  ) {
